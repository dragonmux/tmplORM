	...
	namespace types
	{
		inline namespace baseTypes
		{
			...
			public:
				...
				constexpr ormDate_t(const uint16_t year, const uint16_t month, const uint16_t day) :
			...
			public:
				...
				constexpr ormDateTime_t(const uint16_t year, const uint16_t month, const uint16_t day, const uint16_t hour,
					const uint16_t minute, const uint16_t second, const uint32_t nanoSecond) :
				...
				constexpr uint16_t nanoSecond() const noexcept { return _nanoSecond; }

				ormDateTime_t(const char *dateTime) noexcept : ormDateTime_t()
				{
					_year = toInt_t<uint16_t>(dateTime, 4);
					dateTime += 5;
					_month = toInt_t<uint16_t>(dateTime, 2);
					dateTime += 3;
					_day = toInt_t<uint16_t>(dateTime, 2);
					dateTime += 3;
